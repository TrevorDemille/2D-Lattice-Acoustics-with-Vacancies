clear all;
%computation time including first two 'for' loops is approx. 1.5 hours
A_o = input('Enter Intial Amplitude: ');
vacsize = input('Enter Vacancy Size: ');
vacnum = input('Enter Total Number of Vacancies: ');
al = input('Enter Attenuation Coefficient Value: ');
omega = input('Enter Angular Frequency: ');
for vn = 1:10 
    vacnum = vacnum+1;
for om = 1:10 %record increasing angular frequency values for each vacancy number
    omega = omega+1.5;
    alpha = 0.00002*al*(0.01*omega);
...
fid = fopen('DrivenData.txt','a'); %Open the file for recording and stockpiling data
%
if LeadAverage ~= 0  
